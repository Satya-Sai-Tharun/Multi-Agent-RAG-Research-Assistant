...
class QueryPlannerAgent(BaseAgent):
    ...
    async def run(self, query: str) -> dict:
        ...
        # Step 3: Refine each sub-query for better retrieval
        refined_queries = []
    ...
    async def _decompose(self, query: str) -> list[str]:
        ...
        try:
            raw = await ollama_client.generate(prompt, temperature=0.0)
    ...
    async def _decide_strategy(self, query: str) -> RetrievalStrategy:
        ...
        try:
            raw = (await ollama_client.generate(prompt, temperature=0.0)).strip().lower()
            if "deep" in raw:
                return "deep"
        except Exception as e:
    ...
    async def _refine_query(self, query: str) -> str:
        ...
        try:
            refined = (await ollama_client.generate(prompt, temperature=0.0)).strip()
